...
class GitCommitSha
  ...
  # Assumes the git CLI is available. This is not the case in production
  # on Heroku.
  def self.retrieve_sha_from_git
    `git rev-parse HEAD 2>/dev/null`.to_s.strip
  end

  # Assumes a .source_version file with SHA inside. A special Heroku buildpack
  # creates this for us in production.
  def self.retrieve_sha_from_file
    filepath = Rails.root.join(".source_version")
    File.exist?(filepath) ? File.read(filepath).to_s.strip : nil
  end
end
