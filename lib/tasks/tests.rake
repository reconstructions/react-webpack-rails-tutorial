if %w[development test].include? Rails.env
  ...
  namespace :tests do
    ...
    namespace :react do
      ...
      desc "Run all JS tests at a specified path.  Example:\n" \
      "rake tests:react:path" \
      "[comments/components/CommentBox/CommentList/CommentList.spec.jsx]"
      task :path, :dir do |_t, args|
        puts Rainbow("\nrake tests:react:path[#{args[:dir]}]\n").blueviolet
        sh "cd client && yarn run test:path -- 'app/bundles/#{args[:dir]}'"
      end

      desc "Run all JS tests at a specified path with debugging. Example:\n" \
      "rake tests:react:path_debug" \
      "[comments/components/CommentBox/CommentList/CommentList.spec.jsx]"
